...
Function Calcula_Preco(vCred)
    ...
    If Min = 0 Then
        ...
        Calcula_Preco = Hora * Moeda
    Else
        If Hora <> 0 Then
            Calcula_Preco = Moeda / 2
            Calcula_Preco = (Hora * Moeda) + Calcula_Preco
        Else
            Calcula_Preco = Moeda / 2
        End If
    End If
...
Function Calcula_Moeda(vVal)
    ...
    pos = InStr(valor, ",")
    If pos <> "0" Then
        Moeda = Mid(valor, pos + 1)
    Else
        Moeda = ""
    End If
    
    If Len(Moeda) = 1 Then
        Calcula_Moeda = valor & "0"
    
    ElseIf Len(Moeda) = 0 Then
        Calcula_Moeda = valor & ",00"
    Else
        Calcula_Moeda = valor
    End If
...
Function FormataMoeda(pvalor)
    ...
    If pvalor = "" Or IsNull(pvalor) = True Then
        FormataMoeda = ""
    Else
...
Function FormataCred(vCred)
    ...
    If Len(vCred) = 4 Then
        HoraTemp = Mid(vCred, 1, 2)
        MinTemp = Mid(vCred, 3, 2)
        
        FormataCred = HoraTemp & ":" & MinTemp
        
    ElseIf Len(vCred) = 5 Then
        HoraTemp = Mid(vCred, 1, 2)
        MinTemp = Mid(vCred, 4, 2)

        FormataCred = HoraTemp & ":" & MinTemp

    Else
        VPStrBox = MsgBox("Formato do crédito está incorreto.", vbCritical, "Guide System - Aviso de erro")
        VGStrDataErro = "sim"
    End If
End Function
